...
RETURNS TABLE (name VARCHAR, num_free_registrations INT, sale_end_date DATE, price NUMERIC(36,2))
AS $$
BEGIN
    RETURN QUERY 
	SELECT Course_packages.name, Course_packages.num_free_registrations, Course_packages.sale_end_date, Course_packages.price
    FROM Course_packages
    WHERE Course_packages.sale_start_date <= CURRENT_DATE AND Course_packages.sale_end_date >= CURRENT_DATE;
END;
$$ LANGUAGE plpgsql;
...
AS $$
BEGIN
    INSERT INTO Buys(buys_date, num_remaining_redemptions, cust_id, number, package_id)
    VALUES (CURRENT_DATE, 
			(SELECT num_free_registrations FROM Course_packages WHERE package_id = input_package_id), 
			input_cust_id, 
			(SELECT number FROM Customers WHERE cust_id = input_cust_id), 
			input_package_id);
END;
$$ LANGUAGE plpgsql;